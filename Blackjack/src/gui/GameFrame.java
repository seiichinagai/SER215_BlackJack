...
/*
 * GameFrame is the main GUI component to the BlackJack game.
...
public class GameFrame {
	...
	public final ImageIcon cardBack =
			new ImageIcon(this.getClass().getResource("/resources/cardBack_blue5.png"));
	...
	private void initialize() {
		...
		// Load player profile from ProfileScreen
		...
		/**
		 * The betField FocusListener resets the betField
		 *  text each hand.
		 */
		...
		lblNumberOfDecks.setBounds(10, 536, 128, 14);
		//frame.getContentPane().add(lblNumberOfDecks);

		numDecksTextField = new JTextField();
		numDecksTextField.setBounds(123, 536, 86, 20);
		...
		/**
		 * betButton provides the ActionListener needed to parse bets from the player.
		 */
		...
		betButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				// Place Bet
				if (validBet()) {
					//If the bet is valid, allow further gameplay
					...
					//If the bet is invalid, notify the user of this
			 ...
			 * The bet is invalid if it cannot be successfully
			 * parsed from a String to an Integer, or if the bet
			 * placed is either less than 1 or greater than the
			 * user's total funds (bank).
			 * 
			...
			private boolean validBet() {
				...
				//If it is valid, remove bet amount from bank.
		...
		/**
		 * numDecksButton provides the ActionListener necessary to set the number of decks to play with.
		 * If the entered number fails to parse from String to Integer, the text field displays an
		 * error message. The error message is also displayed if the user enters a number less
		 * than 1 or greater than 8.
		 */
		...
		numDecksButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//If the number of decks entered was valid, update corresponding fields and labels.
				if (shoeSet) {
					s.setDecks(number);
					JLabel numDecksLabel = new JLabel(""+number);
					numDecksLabel.setBounds(115, 536, 86, 20);
		...
		numDecksButton.setBounds(219, 536, 129, 23);
		...
		/**
		 * standButton provides the ActionListener necessary to indicate they would like to stand. Play
		 * is finished up with the dealer, and then the game state is evaluated.
		 */
		standButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				/**
				 * The switch structure below appropriates the corresponding funds based on
				 * the winner of the hand. Also, the results message String is set for each
				 * winner case, as well as stats being updated.
				 */
				switch (winner) {
				case 1: { // Player won
		...
		/**
		 * hitButton provides the necessary ActionListener to allow the player (and dealer) to draw
		 * cards and continue gameplay.
		 */
		hitButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				// If Player bust, end player-turn.
		...
		/**
		 * dealButton provides the ActionListener necessary to start the BlackJack game.
		 */
		dealButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				//Set appropriate buttons at this juncture to be visible or not.
				dealButton.setVisible(false);
		...
		/**
		 * btnContinue provides the ActionListener necessary to move from the end of a game to the
		 * results screen, once the player is ready.
		 */
		btnContinue.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				// If they have, they can choose to exit or play a new game.
				if (p.getBank() == 0) {
					newGameDecision();
					bankLabel.setText("" + p.getBank());
					lblTGames.setText("Total Games: " + p.getTotalGames());
	 ...
	 * Conversely, they may exit if they do not wish to start a new game.
	 */
